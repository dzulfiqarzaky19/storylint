...
8f. **An instruction that only a human can execute is unprotectable.** Keep instructional comments few and adjacent to the code they describe; prefer a check over a comment wherever a check is possible. Remote/duplicated prose rules rot (E1-2 inverted networkidle comment). Mechanical gates raise the floor, not the ceiling. ([unprotectable-instructions](./unprotectable-instructions.md))
