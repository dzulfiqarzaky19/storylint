...
8g. **A test or lock TITLE may claim only what its assertions can establish.** A source-text assertion may say a mechanism is present; it may not claim a rendered outcome, because it never renders. Titles are read far more often than bodies, so an overpromising name is load-bearing misinformation: `MUTATION lock: face-tab clip fix keeps selected label fully on-screen` reads the source as strings and cannot fail on a clipped label — its title nearly closed a real finding (T-010) as a duplicate. Extends 8e from report lines to check names. (rat scar, dolphin catch, 2026-08-01; T-011)
